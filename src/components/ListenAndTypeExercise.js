// src/components/ListenAndTypeExercise.js - Deep Debug Analysis and Fix
...
function ListenAndTypeExercise({ onBack, onLogoClick }) {
  ...
  // CRITICAL FIX: Completely rewritten playAudio to be more robust
  const playAudio = useCallback(() => {
    ...
    // Set playing state BEFORE starting play
    setIsPlaying(true);
    audio.currentTime = 0;
  ...
  const moveToNextQuestion = useCallback(() => {
    ...
    if (currentQuestion + 1 < testSentences.length) {
      // CRITICAL: Reset ALL audio states cleanly
  ...
  // CRITICAL FIX: Completely rewritten audio event handlers
  useEffect(() => {
  ...
  }, [currentData, currentQuestion]); // Only depend on currentData and currentQuestion

  // CRITICAL FIX: Simplified auto-play with better state management
  useEffect(() => {
    // Only auto-play if we've started, have data, no results showing, and haven't played yet
  ...
  }, [hasStarted, currentData, showResults, currentQuestion]); // Removed playCount to avoid circular dependencies
          ...
          <div className="audio-section-compact">
            ...
            <div className="audio-controls-compact">
              <button 
                ...
                onMouseEnter={(e) => {
                  if (!isPlaying && playCount < 3 && !audioError) {
                    e.target.style.background = 'linear-gradient(135deg, #38a169, #2f855a)';
              ...
              {!audioError && (
                <div style={{
                  ...
                  <div style={{ fontSize: '0.7em', color: '#999', marginTop: '5px' }}>
                    Q{currentQuestion + 1} | Playing: {isPlaying ? 'YES' : 'NO'} | Count: {playCount}
                  </div>
                </div>
              )}
...
export default ListenAndTypeExercise; normaliseText = (text) => {
    return text.toLowerCase()
      .replace(/[.,!?;:"']/g, '')
      .replace(/\s+/g, ' ')
      .trim();
